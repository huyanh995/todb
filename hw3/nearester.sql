...
RADIUS_SEARCH(RADIUS) AS (
    ...
    WHERE NOT EXISTS (
        ...
        FROM LOCATIONS
        WHERE
            db2gse.ST_WITHIN(GEOLOCATION, db2gse.ST_BUFFER(db2gse.ST_POINT(40.891720, -73.016479, 1), RADIUS, 'KILOMETER')) = 1
    )
)
...
SELECT
    ...
    db2gse.ST_DISTANCE(GEOLOCATION, db2gse.ST_POINT(40.891720, -73.016479, 1), 'KILOMETER') AS DISTANCE_KILOMETER
FROM RADIUS_SEARCH
JOIN LOCATIONS ON db2gse.ST_WITHIN(GEOLOCATION, db2gse.ST_BUFFER(db2gse.ST_POINT(40.891720, -73.016479, 1), RADIUS, 'KILOMETER')) = 1
...
-- FROM LOCATIONS
-- WHERE
--     db2gse.ST_WITHIN(GEOLOCATION, db2gse.ST_BUFFER(db2gse.ST_POINT(40.891720, -73.016479, 1), 10, 'KILOMETER')) = 1
-- ORDER BY DISTANCE_KILOMETER
-- LIMIT 1;